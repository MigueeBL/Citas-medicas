...
function exportarPDF(datos) {
  ...
  function seccion(titulo) {
    ...
    doc.setTextColor(...BL); doc.setFontSize(11); doc.setFont("helvetica", "bold");
    doc.text(titulo, 14, y + 1);
    y += 10;
  }
  function fila(label, value, alt = false) {
    if (alt) { doc.setFillColor(248, 250, 252); doc.rect(10, y - 4, 190, 8, "F"); }
    doc.setTextColor(...BK); doc.setFontSize(10); doc.setFont("helvetica", "normal");
    doc.text(label, 16, y + 1); doc.setFont("helvetica", "bold");
    doc.text(String(value), 120, y + 1);
    y += 9;
  }
  function linea() { doc.setDrawColor(220, 220, 220); doc.line(10, y, 200, y); y += 6; }
  ...
  fila("Ingresos estimados", `$${ingresosMes}`, true);
  y += 4; linea();
  ...
  if (especialidadesAprobados.length === 0) {
    ...
    doc.text("Sin medicos aprobados", 16, y); y += 10;
  } else {
    especialidadesAprobados.forEach((e, i) => fila(e.name, `${e.value} medico(s)`, i % 2 !== 0));
  }
 
  doc.setFillColor(...BL); doc.rect(0, 282, 210, 15, "F");
...
function StatCard({ label, value, color, bg, sub }) {
  return (
    <div style={{ background: bg, borderRadius: 12, padding: "14px 16px", flex: 1, minWidth: 120 }}>
      <div style={{ fontSize: 26, fontWeight: 700, color }}>{value}</div>
      <div style={{ fontSize: 12, color, marginTop: 2, fontWeight: 500 }}>{label}</div>
      {sub && <div style={{ fontSize: 10, color, opacity: 0.7, marginTop: 4 }}>{sub}</div>}
    </div>
  );
}
...
export default function Estadisticas() {
  ...
  const [conteos, setConteos]     = useState({ medicos: 0, medicosAprobados: 0, medicosPendientes: 0, pacientes: 0, asistentes: 0 });
  ...
  async function fetchEstadisticas() {
    ...
    try {
      ...
      setConteos({ medicos: medicos.length, medicosAprobados: medicosAprobados.length, medicosPendientes: medicosPendientes.length, pacientes: pacientes.length, asistentes: asistentes.length });
 
      const espTodos = {};
      medicos.forEach((m) => { const e = m.especialidad || "Sin especialidad"; espTodos[e] = (espTodos[e] || 0) + 1; });
      setEspecialidadesTodos(Object.entries(espTodos).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value));
 
      const espAprobados = {};
      medicosAprobados.forEach((m) => { const e = m.especialidad || "Sin especialidad"; espAprobados[e] = (espAprobados[e] || 0) + 1; });
      setEspecialidadesAprobados(Object.entries(espAprobados).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value));
      ...
      setIngresosMes(citasMes.reduce((acc, c) => acc + (c.monto || 0), 0));
 
      const estadoMap = {};
      citasMes.forEach((c) => { const e = c.estado || "sin estado"; estadoMap[e] = (estadoMap[e] || 0) + 1; });
      setCitasPorEstado(Object.entries(estadoMap).map(([estado, total]) => ({ estado, total })));
    } catch (err) {
  ...
  const datosExport = { conteos, especialidadesTodos, especialidadesAprobados, ingresosCitas, ingresosMes };
  const chartCols   = isMobile || isSmall ? "1fr" : "1fr 1fr";
  const padding     = isMobile ? "48px 12px 12px" : isSmall ? 16 : 24;
 
  if (loading) return <div style={styles.loading}>Cargando estadísticas...</div>;
 
  return (
    <div style={{ ...styles.page, padding }}>
      {/* Encabezado */}
      <div style={{ ...styles.header, flexDirection: isMobile ? "column" : "row", gap: isMobile ? 10 : 0 }}>
        <div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button style={styles.btnSecundario} onClick={() => exportarCSV(datosExport)}>📥 CSV</button>
          <button style={styles.btnPrimario}   onClick={() => exportarPDF(datosExport)}>📄 PDF</button>
        </div>
      </div>
      ...
      <SectionCard title="Usuarios registrados">
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <StatCard label="Médicos totales"    value={conteos.medicos}           color={BLUE}      bg="#E6F1FB" sub={`${conteos.medicosAprobados} aprobados · ${conteos.medicosPendientes} pendientes`} />
      ...
      <SectionCard title="Ingresos del mes">
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <StatCard label="Ingresos estimados" value={`$${ingresosMes.toLocaleString("es-MX")}`} color="#27500A" bg="#EAF3DE" sub="Suma del campo monto" />
        </div>
      </SectionCard>
 
      {/* Gráficas especialidades */}
      <div style={{ display: "grid", gridTemplateColumns: chartCols, gap: 14, marginBottom: 14 }}>
        <SectionCard title="Especialidades — todos">
          {especialidadesTodos.length === 0 ? <p style={styles.empty}>Sin datos</p> : (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={especialidadesTodos} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value">
                  {especialidadesTodos.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Pie>
        ...
        <SectionCard title="Especialidades — aprobados">
          {especialidadesAprobados.length === 0 ? <p style={styles.empty}>Sin médicos aprobados</p> : (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={especialidadesAprobados} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value">
                  {especialidadesAprobados.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Pie>
                <Tooltip formatter={(v, n) => [`${v} médicos`, n]} contentStyle={styles.tooltip} />
                <Legend iconType="circle" iconSize={8} formatter={(v) => <span style={{ fontSize: 11, color: "#555" }}>{v}</span>} />
              </PieChart>
      ...
      {citasPorEstado.length > 0 && (
        <SectionCard title="Citas por estado — mes actual">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={citasPorEstado} barSize={36}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="estado" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#888" }} />
              <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#888" }} allowDecimals={false} />
              <Tooltip formatter={(v) => [`${v} citas`, "Total"]} contentStyle={styles.tooltip} />
              <Bar dataKey="total" radius={[6, 6, 0, 0]}>
                {citasPorEstado.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </SectionCard>
...
const styles = {
  ...
  btnPrimario:  { padding: "9px 14px", borderRadius: 8, border: "none", background: BLUE, color: "white", fontSize: 13, cursor: "pointer", fontWeight: 500 },
  btnSecundario:{ padding: "9px 14px", borderRadius: 8, border: `1px solid ${BLUE}`, background: "white", color: BLUE, fontSize: 13, cursor: "pointer", fontWeight: 500 },
  card:         { background: "white", borderRadius: 12, border: "0.5px solid #e5e7eb", padding: "16px 18px", marginBottom: 14 },
  cardTitle:    { fontSize: 14, fontWeight: 600, color: "#1a1a2e", marginBottom: 14, marginTop: 0 },
